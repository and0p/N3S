...
public:
	...
	bool init(HWND hwnd, long sampleRate);   // initialize the primary and secondary sound buffer
...
bool DirectSound::init(HWND hwnd, long sampleRate)
{
	HRESULT hr;
	DWORD freq;
	...
	if (FAILED(hr = pDirectSound->SetCooperativeLevel(hwnd, DSSCL_EXCLUSIVE))) {
